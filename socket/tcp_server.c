...
tcp_server_addr.sin_addr.s_addr=inet_addr(127001);
/*######################################################## */
if((socket_id=socket(PF_INET,SOCK_STREAM,0))<0)
	perror("Failed to initialize the Server Socket");
if(bind(socket_id,(struct sockaddr *)&tcp_server_addr,sizeof(tcp_server_addr))==-1)
	perror(" Failed to bind the Server Socket to the port : LOCAL_PORT ");
